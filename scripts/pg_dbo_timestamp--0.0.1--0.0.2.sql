...
BEGIN
    BEGIN
        RETURN QUERY SELECT t.type, t.schema, t.name, t.identity FROM pg_identify_object(classid, objid, 0) t;
...
ALTER TABLE ONLY dbots_event_data
	ALTER COLUMN cur_user SET DEFAULT "current_user"();

ALTER TABLE dbots_event_data
	ADD COLUMN ses_user name;

ALTER TABLE ONLY dbots_event_data
	ALTER COLUMN ses_user SET DEFAULT "session_user"();
